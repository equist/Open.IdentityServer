...
public class EndSessionTests
{
    ...
    private readonly Client _wsfedClient;
    ...
    {
        await _mockPipeline.LoginAsync("bob");
        _ = _mockPipeline.GetSessionCookie().Value;
    ...
    public async Task logout_should_invoke_back_channel_logout()
    {
        _mockPipeline.BackChannelMessageHandler.OnInvoke = async req =>
        {
            ...
            var payload = JsonElement.Parse(json);
            payload.GetProperty("iss").GetString().Should().Be("https://server");
            payload.GetProperty("sub").GetString().Should().Be("bob");
            payload.GetProperty("aud").GetString().Should().Be("client3");
            payload.GetProperty("iat").Should().NotBeNull();
            payload.GetProperty("jti").Should().NotBeNull();
            payload.GetProperty("sid").Should().NotBeNull();
            payload.GetProperty("events").ValueKind.Should().Be(JsonValueKind.Object);

            var events = payload.GetProperty("events");
            events.GetPropertyCount().Should().Be(1);
            events.GetProperty("http://schemas.openid.net/event/backchannel-logout").Should().NotBeNull();
            events.GetProperty("http://schemas.openid.net/event/backchannel-logout").ValueKind.Should().Be(JsonValueKind.Object);

            var evt = events.GetProperty("http://schemas.openid.net/event/backchannel-logout");
            evt.GetPropertyCount().Should().Be(0);
        };
    ...
    {
        _mockPipeline.BackChannelMessageHandler.OnInvoke = _ => throw new Exception("boom!");
