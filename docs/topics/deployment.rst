...
^^^^^^^^^^^^^^^^^^^^
Typically you will design your Open.IdentityServer deployment for high availability:

.. image:: images/deployment.png

Open.IdentityServer itself is stateless and does not require server affinity - but there is data that needs to be shared between the instances.
...
Operational data
^^^^^^^^^^^^^^^^
For certain operations, Open.IdentityServer needs a persistence store to keep state, this includes:
...
You can also implement support for your own custom storage mechanism by implementing ``IPersistedGrantStore`` - by default Open.IdentityServer injects an in-memory version.
